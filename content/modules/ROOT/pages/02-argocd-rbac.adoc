...
---
 oc get argocd argocd -n {user}-argocd -o=jsonpath="{.spec.rbac}" | jq .
---
...
to avoid ambiguity but here we follow Argo CD defaults.

Now let's look at what groups you are member of, in the Argo CD user interface select the User Info section in the left
sidebar:

image::argocd-groups.png[]

Notice you are not a member of the cluster-admins group, rather you are a member of the `team{user-num}`
group. Since the Argo CD RBAC we looked at earlier do not include these groups this means you do not have any access to
...
[source,sh,subs="attributes",role=execute]
----
oc apply -n {user}-argocd -f ~/workshop/content/modules/ROOT/examples/rbac/argocd.yaml | sed "s/%USER%/{user}/"
----
...
----
  rbac:
    ...
    policy: |
      g, system:cluster-admins, role:admin
      g, cluster-admins, role:admin
      g, team{user-num}, role:admin
    scopes: '[groups]'
----

Notice a new group has been added granting users in your team group the `admin` role.

We can also define our own policies here if we need to have more fine-grained permissions. We will look at the more
later when talking about Argo CD Projects and RBAC but first we need to understand what an Argo CD Project is and
why it is needed.
