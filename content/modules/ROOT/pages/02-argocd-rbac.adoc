...
Notice the new group adding team{usernum} to the list of groups who can act as an administrator.

However notice that we change the `defaultPolicy` to `role:none` and we define this role with a policy (p) rather then a group (g):
...
This is a simple policy that denies access to everything, when we look into Argo CD Projects and RBAC in subsequent sections we will break down
the policy definition in much more detail.
