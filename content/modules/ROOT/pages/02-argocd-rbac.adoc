...
spec:
  ...
  - namespace: user2-dev
    server: https://kubernetes.default.svc
  - namespace: user2-stage
    server: https://kubernetes.default.svc
  - namespace: user2-prod
    server: https://kubernetes.default.svc
  roles:
...
Notice that the `workshop` Project limits Applications to a specific set of namespaces, the `{user}-dev`, `{user}-stage` and `{user}-prod` namespaces. We also
limited the destinations to the local Kubernetes server with `server: https://kubernetes.default.svc`.

[NOTE]
You can wildcard the namespaces and we could have had a single destination using `{user}-*`, however we do not want to have `{user}-argocd` as a valid destination.

This limitation on destinations ensures that Applications in this Project cannot deploy resources to other namespaces
even though the underlying Argo CD application-controller may have permissions to do so. This is recommended when configuring
Argo CD for multi-tenant use cases.
...
this is typically the purview of the platform team to manage these resources.
...
Here we are defining a single role, `admin`, that is associated with the group `team2`. We use the policies
section to add specific permissions to the `admin` role. Let's break
...
The second policy line enables usage of the terminal in the Argo CD user interface

Now let's try to create Application again.
