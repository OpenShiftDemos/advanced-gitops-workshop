...
credential so that Argo CD RBAC is applied. The second way is to use the `--core` switch which logs in using
...
our terminal is running in a pod in OpenShift this is not possible. There here we will
