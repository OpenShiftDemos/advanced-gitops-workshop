...
To understand the importance of health checks, let's deploy the bgd Application again but this time for illustrative purposes with a PersistentVolumeClaim. Also there is a sync wave
defined with the PVC being the first resource deployed. In the link:https://argocd-server-{user}-argocd.{openshift_cluster_ingress_domain}[Argo CD UI, window="_blank"]
