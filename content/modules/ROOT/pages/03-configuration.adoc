...
To understand the importance of health checks, let's deploy the bgd Application again but this time with a PersistentVolumeClaim. In
the link:https://argocd-server-{user}-argocd.{openshift_cluster_ingress_domain}[Argo CD UI, window="_blank"] create a new Application as follows:

[.console-input]
[source,yaml,subs="attributes",role=execute]
----
sed "s/%USER%/{user}/" ~/workshop/content/modules/ROOT/examples/health/bgd/base | oc apply -f -
----
