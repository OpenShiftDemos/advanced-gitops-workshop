...
Before we delve into ApplicatioSets, let's dive into the useful link:link:https://argo-cd.readthedocs.io/en/latest/operator-manual/cluster-bootstrapping/#app-of-apps-pattern[App-of-Apps, window="_blank"] 
...
Next deploy the app-of-apps Application:

[.console-input]
[source,yaml,subs="attributes",role=execute]
----
sed "s/%USER%/{user}/" ~/workshop/content/modules/ROOT/examples/app-of-apps/coolstore-app-of-apps.yaml | oc apply -f -
----
